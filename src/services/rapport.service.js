...
// Génération PDF du rapport à partir des articles sélectionnés et des critères de filtrage
...
async function genererPDF({ articles, dateDebut, dateFin, periode, categorie, zone, idNote }) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(BLANC).fontSize(22).font('Helvetica-Bold')
       .text('DISVI — Dispositif de Veille Stratégique', 50, 30, { width: largeur });
    doc.fontSize(13).font('Helvetica').text('Note de Synthèse', 50, 60);
    ...
    doc.fillColor(BLEU_MARINE).fontSize(13).font('Helvetica-Bold').text('Statistiques', 50, doc.y);
    ...
    // ─── Articles ─────────────────────────────────────────────────────────────
    doc.fillColor(BLEU_MARINE).fontSize(13).font('Helvetica-Bold').text('Articles sélectionnés', 50, doc.y);
    doc.rect(50, doc.y + 2, largeur, 1).fill(BLEU_MARINE);
    doc.moveDown(1);
    ...
    for (let i = 0; i < articles.length; i++) {
      ...
      doc.fillColor(BLEU_MARINE).fontSize(9).font('Helvetica-Bold').text(`${i + 1}.`, 55, yArt + 4, { width: 20 });
      doc.fillColor(BLEU_CLAIR).fontSize(9).font('Helvetica-Bold').text(art.nom_source || '', 75, yArt + 4, { width: 200 });

      const dateStr = art.date_publication ? new Date(art.date_publication).toLocaleDateString('fr-FR') : '';
      doc.fillColor(GRIS_FONCE).fontSize(8).font('Helvetica').text(dateStr, 50 + largeur - 80, yArt + 5, { width: 80, align: 'right' });

      if (art.zone) {
        const badgeColor = art.zone === 'nationale' ? '#27AE60' : '#8E44AD';
        doc.rect(50 + largeur - 170, yArt + 3, 80, 12).fill(badgeColor);
        doc.fillColor(BLANC).fontSize(7).font('Helvetica').text(art.zone.toUpperCase(), 50 + largeur - 170, yArt + 5, { width: 80, align: 'center' });
      }

      doc.moveDown(0.1);
      doc.fillColor(GRIS_FONCE).fontSize(10).font('Helvetica-Bold').text(art.titre || '', 55, doc.y + 4, { width: largeur - 10 });
      doc.moveDown(0.4);

      const texteResume = art.resume_auto || art.description || '';
      if (texteResume) {
        doc.fillColor('#555555').fontSize(9).font('Helvetica').text(texteResume.slice(0, 400), 55, doc.y, { width: largeur - 10 });
        doc.moveDown(0.3);
      }

      if (art.categories && art.categories.length > 0 && art.categories[0] !== '') {
        doc.fillColor(ORANGE).fontSize(8).font('Helvetica-Oblique').text(`Catégories : ${art.categories.join(', ')}`, 55, doc.y);
        doc.moveDown(0.3);
      }

      if (art.url_origine) {
        doc.fillColor(BLEU_CLAIR).fontSize(7).font('Helvetica').text(art.url_origine.slice(0, 80), 55, doc.y, { link: art.url_origine });
        doc.moveDown(0.3);
      }
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.fillColor(BLANC).fontSize(8).font('Helvetica')
         .text(
           `DISVI — Dispositif de Veille Stratégique IE237  |  Page ${i - range.start + 1} / ${range.count}`,
